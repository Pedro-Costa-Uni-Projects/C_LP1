...
int json_is_object(char ch) { return ch == '{'; }

int json_is_object_end(char ch) { return ch == '}'; }

int json_is_array(char ch) { return ch == '['; }

int json_is_array_end(char ch) { return ch == ']'; }
/**
...
void detetaErros(contadorElementos resultado) {
  if ((resultado.num_ARRAY == 0) && (resultado.num_OBJECT == 0) &&
...
int detetaStackOverflow(contadorElementos resultado) {
  if ((resultado.num_ARRAY == -1) || (resultado.num_OBJECT == -1) ||
